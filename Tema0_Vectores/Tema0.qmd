...
Las componentes de los vectores nos van a permitir un método sencillo pero general para sumar vectores. Supongamos que tenemos un sistema cartesiano de ejes de coordenadas. Entonces $A_x$ es la magnitud del vector $\vec{A_x}$ y $A_y$ es la magnitud del vector $\vec{A_y}$. $A_x$ y $A_y$ son las componentes del vector $\vec{A}$. Podemos calcular las componentes del vector $\vec{A}$ si conocemos la magnitud y la dirección de dicho vector. Describiremos la dirección de un vector con su ángulo $\theta$, relativo a una dirección de referencia, que es el eje $x$ positivo, siempre en sentido antihorario
...
El producto vectorial de los vectores $\vec{A}$ y $\vec{B}$ se denota como $\vec{A} \wedge \vec{B}$. Definimos dicho producto vectorial como un vector, perpendicular al plano que forman los vectores $\vec{A}$ y $\vec{B}$, y cuya magnitud viene dada por $$|\vec{A} \wedge \vec{B}| = |\vec{A}|  |\vec{B}|  \text{sen} \theta  ,$$ siendo $\theta$ el ángulo que forman ambos vectores. La dirección y sentido del vector $\vec{A} \wedge \vec{B}$ nos lo da la *regla de la mano derecha*. La dirección del producto vectorial está definida por la dirección del pulgar, cerrando los demás dedos en torno al vector $\vec{A}$ primero y siguiendo con el vector $\vec{B}$.
...
$$\hat{\imath} \wedge \hat{\jmath} = \hat{k} \quad \; \hat{\jmath} \wedge \hat{\imath} = -\hat{k} \quad \; \hat{\imath} \wedge \hat{\imath} = 0$$
...
$$
\begin{aligned}
\vec{A} \wedge \vec{B} &= \left ( A_x \hat{\imath} + A_y \hat{\jmath} + A_z \hat{k} \right ) \wedge \left (B_x \hat{\imath} + B_y \hat{\jmath} + B_z \hat{k} \right )
...
$$ \vec{A} \wedge \vec{B} = \left | \begin{matrix} \hat{\imath} & \hat{\jmath} & \hat{k} \\ A_x & A_y & A_z \\ B_x & B_y & B_z \end{matrix} \right |$$

:::: {.callout-note title="Ejemplo" collapse="false" icon="false"}

Sea $\vec{A}= 6  \hat{\imath}$ y $\vec{B}= 4 \left ( \cos 30^\circ  \hat{\imath} + \text{sen}  30^\circ  \hat{\jmath} \right )$. Calcula $\vec{A} \wedge \vec{B}$.

::: {.callout-note title="Solución" collapse="true" icon="false"}

$$\vec{A} \wedge \vec{B} = 24 \cdot \text{sen}  30^\circ  \hat{k} = 12  \hat{k}$$
:::
